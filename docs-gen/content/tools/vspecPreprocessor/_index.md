...
* Create overlay for a truck. The Car tree is used as the base vspec tree. The vspecPreprocessor generates one truck specific overlay file,
another that adds the axle feature signals must also be included, which is done in the "overlay" directive in the vehicleConfig-truck.json file.
The vspecPreprocessor generates the overlay file by the command below:
```
$ python3 vspecPreprocessor.py -i Config/Truck/vehicleConfig-truck.json -o Config/Truck/truck.vspec -s Config/Truck/configScope.json -v Vehicle/Car/VehicleSignalSpecification.vspec

Overlay configuration saved to Config/Truck/truck.vspec

Exporter command: vspec export yaml -u Vehicle/Car/units.yaml -q Vehicle/Car/quantities.yaml -l Config/Truck/truck.vspec  -l Config/Truck/Overlays/Chassis.vspec -l Config/Truck/Overlays/Default.vspec -s Vehicle/Car/VehicleSignalSpecification.vspec -o cvis.yaml
```
The exporter command displayed above can then be copied and applied to the vspec exporter.
...
Besides the overlay file that the vspecPreprocessor generates it is possible to instruct it to include other overlay files in the command to the VSS-tools exporter.
This is done by adding another object to the "configurations" array as shown in the example below.
```
        {
            "overlays": [
                "Config/Truck/Overlays/Chassis.vspec",
                "Config/Truck/Overlays/Default.vspec"
            ]
        }
```
These overlays can be used to modify existing branches as in the Truck example shown above, or to add or remove branches/nodes, or to initialise default values, etc.
