...
another that adds the axle feature signals must also be included in the command to the VSS-tools exporter.
This overlay is available in the Profile/Truck/Chassis directory.
The first overlay is generated by the command below:
```
...
Exporter command: vspec export yaml -u Vehicle/Car/units.yaml -q Vehicle/Car/quantities.yaml -l Config/Truck/truck.vspec -s Vehicle/Car/VehicleSignalSpecification.vspec -o cvis.yaml
```
The VSS-tools exporter command displayed by the vspecPreprocessor does not include the second overlay so it has to be inserted manually to get the final exporter command:
```
vspec export yaml -u Vehicle/Car/units.yaml -q Vehicle/Car/quantities.yaml -l Config/Truck/truck.vspec -l Profile/Truck/Chassis/Chassis.vspec -s Vehicle/Car/VehicleSignalSpecification.vspec -o cvis.yaml
```
